...
int particleFilter(unsigned char * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles) {
    ...
    char * tempchar = "./particle_float.cl";
    ...
    objxy_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof (int) *countOnes, NULL, &err);
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueWriteBuffer(cmd_queue, objxy_GPU, 1, 0, sizeof (int) *countOnes, objxy, 0, 0, 0);
    if (err != CL_SUCCESS) {
