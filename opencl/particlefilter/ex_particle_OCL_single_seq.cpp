...
int particleFilter(unsigned char * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles) {
    ...
    CDF_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof (float) * Nparticles, NULL, &err);
    if (err != CL_SUCCESS) {
    ...
    partial_sums = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof (float) * Nparticles + 1, likelihood, &err);
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueWriteBuffer(cmd_queue, xj_GPU, 1, 0, sizeof (float) *Nparticles, xj, 0, 0, 0);
    if (err != CL_SUCCESS) {
        printf("ERROR: clEnqueueWriteBuffer arrayX_GPU (size:%d) => %d\n", Nparticles, err);
        return -1;
    }
    err = clEnqueueWriteBuffer(cmd_queue, yj_GPU, 1, 0, sizeof (float) *Nparticles, yj, 0, 0, 0);
    if (err != CL_SUCCESS) {
