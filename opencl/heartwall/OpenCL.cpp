...
// *********************************************************************
const char* OpenCL::pe(cl_int error)
{
...
void OpenCL::launch(string toLaunch)
{
	...
	if (ret != CL_SUCCESS)
	{
		printf("\nError attempting to launch %s. Error in clCreateProgramWithSource with error code %s\n\n", toLaunch.c_str(), this->pe(ret) );
		exit(1);
	}
}
...
void OpenCL::buildKernel()
{
	...
	if (ret != CL_SUCCESS)
	{
		printf("\nError at clCreateProgramWithSource! Error code %s\n\n", this->pe(ret) );
		exit(1);
	}
	...
	ret = clBuildProgram(program, devices_n, device_id, NULL, NULL, NULL);
	
	if (ret != CL_SUCCESS)
	{
		printf("\nError at clBuildProgram! Error code %s\n\n", this->pe(ret) );
		cout << "\n*************************************************" << endl;
...
void OpenCL::getDevices(cl_device_type deviceType)
{
	...
	if (ret != CL_SUCCESS)
	{
		printf("\nError at clCreateContext! Error code %s\n\n", this->pe(ret) );
		exit(1);
	}
	...
	if (ret != CL_SUCCESS)
	{
		printf("\nError at clCreateCommandQueue! Error code %s\n\n", this->pe(ret) );
		exit(1);
	}
}
