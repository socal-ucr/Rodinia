...
__kernel void hwKernel(__global float* d_frame,
                       ...
                       __global float* d_endoT,
                       __global float* d_epiT,
                       __global float* d_in2,
                       __global float* d_conv,
                       __global float* d_in2_pad_cumv,
                       __global float* d_in2_pad_cumv_sel,
                       __global float* d_in2_sub_cumh,
                       __global float* d_in2_sub_cumh_sel,
                       __global float* d_in2_sub2,
                       __global float* d_in2_sqr,
                       __global float* d_in2_sqr_sub2,
                       __global float* d_in_sqr,
                       __global float* d_tMask,
                       __global float* d_mask_conv,
                       int d_frame_no,
                       __global int* d_in_pointer,
                       int endoPoints,
                       __constant struct_common_change* d_common_change,
                       __constant struct_common* d_common,
                       __constant struct_unique* d_unique,
                       int localWorksize)
{
	__global float* d_in;
	__global int* d_Row;
	__global int* d_Col;
	__global float* d_T;
	__global int* d_tRowLoc;
	__global int* d_tColLoc;
	...
	__local float in_partial_sum[51];        // WATCH THIS !!! HARDCODED VALUE
	__local float in_sqr_partial_sum[51];    // WATCH THIS !!! HARDCODED VALUE
	__local float in_final_sum;
	__local float in_sqr_final_sum;
	__local float denomT;
	__local float par_max_val[131];          // WATCH THIS !!! HARDCODED VALUE
	__local int par_max_coo[131];            // WATCH THIS !!! HARDCODED VALUE
	__local float d_in_mod_temp[2601];
	...
	// Set up kernel pointers based on thread index.
	if (bx >= endoPoints)
	{
		d_Col = d_epiCol;
		d_Row = d_epiRow;
		d_tRowLoc = d_tEpiRowLoc;
		d_tColLoc = d_tEpiColLoc;
		d_T   = d_epiT;
	} else {
		d_Col = d_endoCol;
		d_Row = d_endoRow;
		d_tRowLoc = d_tEndoRowLoc;
		d_tColLoc = d_tEndoColLoc;
		d_T   = d_endoT;
	}
	...
	if (d_frame_no == 0)
	{
		...
		int tempIndex = d_in_pointer[bx];
		// pointers to: current template for current point
		if (bx >= endoPoints)
		{
			d_in = &d_epiT[tempIndex];
		} else {
			d_in = &d_endoT[tempIndex];
		}

		// UPDATE ROW LOC AND COL LOC

		// update temporary endo/epi row/col coordinates (in each block corresponding to point, narrow work to one thread)
		ei_new = tx;
		if (ei_new == 0)
		{
			// update temporary row/col coordinates
			pointer = d_unique[bx].point_no * d_common->no_frames + d_frame_no;
			if (bx >= endoPoints)
			{
				d_tRowLoc[pointer] = d_Row[d_unique[bx].point_no];
				d_tColLoc[pointer] = d_Col[d_unique[bx].point_no];
			} else {
				d_tRowLoc[pointer] = d_Row[d_unique[bx].point_no];
				d_tColLoc[pointer] = d_Col[d_unique[bx].point_no];
			}
		...
		while (ei_new < d_common->in_elem)
		{
			...
			ori_row = d_Row[d_unique[bx].point_no] - 25 + row - 1;
			ori_col = d_Col[d_unique[bx].point_no] - 25 + col - 1;
	...
	if (d_frame_no != 0)
	{
		...
		in2_rowlow = d_Row[d_unique[bx].point_no] - d_common->sSize; // (1 to n+1)
		in2_collow = d_Col[d_unique[bx].point_no] - d_common->sSize;
		...
		while (ei_new < d_common->in2_elem)
		{
			...
			d_in2[(bx*d_common->in2_elem) + ei_new] = d_frame[ori_col * d_common->frame_rows + ori_row];
		...
		while (ei_new < d_common->conv_elem)
		{
			...
			for (ja = ja1; ja <= ja2; ja++)
			{
				...
				{
					ib = ip1 - ia;
					s = s + d_in_mod_temp[d_common->in_rows * (ja - 1) + ia - 1] * d_in2[(bx*d_common->in2_elem) + d_common->in2_rows
					                                                                     * (jb - 1) + ib - 1];
				}
			}

			//d_unique[bx].d_conv[d_common->conv_rows*(jc-1)+ic-1] = s;
			d_conv[bx*d_common->conv_elem + ei_new] = s;
		...
		while (ei_new < d_common->in2_pad_cumv_elem)
		{
			...
			if (row > (d_common->in2_pad_add_rows - 1) && // do if has numbers in original array
			    row < (d_common->in2_pad_add_rows + d_common->in2_rows) && col > (d_common->in2_pad_add_cols - 1)
			    && col < (d_common->in2_pad_add_cols + d_common->in2_cols))
			{
				...
				d_in2_pad_cumv[bx*d_common->in2_pad_cumv_elem + (ei_new)] = d_in2[(bx*d_common->in2_elem) + ori_col * d_common->in2_rows + ori_row];
			}
			else
			{ // do if otherwise
				d_in2_pad_cumv[bx*d_common->in2_pad_cumv_elem +ei_new] = 0;
			}
		...
		while (ei_new < d_common->in2_pad_cumv_cols)
		{
			...
			{
				d_in2_pad_cumv[bx*d_common->in2_pad_cumv_elem +position] = d_in2_pad_cumv[bx*d_common->in2_pad_cumv_elem +position] + sum;
				sum = d_in2_pad_cumv[bx*d_common->in2_pad_cumv_elem +position];
			}
		...
		while (ei_new < d_common->in2_pad_cumv_sel_elem)
		{
			...
			d_in2_pad_cumv_sel[(bx*d_common->in2_pad_cumv_sel_elem) +ei_new] = d_in2_pad_cumv[bx*d_common->in2_pad_cumv_elem + (ori_col * d_common->in2_pad_cumv_rows
				                                              + ori_row)];
		...
		while (ei_new < d_common->in2_sub_cumh_elem)
		{
			...
			d_in2_sub_cumh[(bx*d_common->in2_sub_cumh_elem) +ei_new] = d_in2_pad_cumv[bx*d_common->in2_pad_cumv_elem + (ori_col * d_common->in2_pad_cumv_rows
				                                      + ori_row)];
		...
		while (ei_new < d_common->in2_sub_cumh_elem)
		{
			...
			// subtract
			d_in2_sub_cumh[(bx*d_common->in2_sub_cumh_elem) +ei_new] = d_in2_pad_cumv_sel[(bx*d_common->in2_pad_cumv_sel_elem) +ei_new]
				- d_in2_sub_cumh[(bx*d_common->in2_sub_cumh_elem) +ei_new];
		...
		while (ei_new < d_common->in2_sub_cumh_rows)
		{
			...
			// loop through all rows
			for (position = pos_ori; position < pos_ori + d_common->in2_sub_cumh_elem; position = position
				     + d_common->in2_sub_cumh_rows)
			{
				d_in2_sub_cumh[(bx*d_common->in2_sub_cumh_elem) +position] = d_in2_sub_cumh[(bx*d_common->in2_sub_cumh_elem) +position] + sum;
				sum = d_in2_sub_cumh[(bx*d_common->in2_sub_cumh_elem) +position];
			}
		...
		while (ei_new < d_common->in2_sub_cumh_sel_elem)
		{
			...
			d_in2_sub_cumh_sel[(bx*d_common->in2_sub_cumh_sel_elem) +ei_new] = d_in2_sub_cumh[(bx*d_common->in2_sub_cumh_elem) +ori_col * d_common->in2_sub_cumh_rows
				            + ori_row];
		...
		while (ei_new < d_common->in2_sub2_elem)
		{
			...
			d_in2_sub2[(bx*d_common->in2_sub2_elem) +ei_new] = d_in2_sub_cumh[(bx*d_common->in2_sub_cumh_elem) +ori_col * d_common->in2_sub_cumh_rows
			                                                                  + ori_row];
		...
		while (ei_new < d_common->in2_sub2_elem)
		{
			...
			// subtract
			d_in2_sub2[(bx*d_common->in2_sub2_elem) +ei_new] = d_in2_sub_cumh_sel[(bx*d_common->in2_sub_cumh_sel_elem) +ei_new] - d_in2_sub2[(bx*d_common->in2_sub2_elem) +ei_new];
		...
		while (ei_new < d_common->in2_sqr_elem)
		{
			...
			temp = d_in2[(bx*d_common->in2_elem) + ei_new];
			d_in2_sqr[(bx*d_common->in2_sqr_elem) +ei_new] = temp * temp;
		...
		while (ei_new < d_common->in2_pad_cumv_elem)
		{
			...
			// execution
			if (row > (d_common->in2_pad_add_rows - 1) && // do if has numbers in original array
			    row < (d_common->in2_pad_add_rows + d_common->in2_sqr_rows) && col > (d_common->in2_pad_add_cols - 1)
			    && col < (d_common->in2_pad_add_cols + d_common->in2_sqr_cols))
			{
				ori_row = row - d_common->in2_pad_add_rows;
				ori_col = col - d_common->in2_pad_add_cols;
				d_in2_pad_cumv[bx*d_common->in2_pad_cumv_elem +ei_new] = d_in2_sqr[(bx*d_common->in2_sqr_elem) +ori_col * d_common->in2_sqr_rows + ori_row];
			}
			else
			{ // do if otherwise
				d_in2_pad_cumv[bx*d_common->in2_pad_cumv_elem +ei_new] = 0;
		...
		while (ei_new < d_common->in2_sub2_elem)
		{
			...
			ori_row = row + d_common->in2_sub_cumh_sel2_rowlow - 1;
			ori_col = col + d_common->in2_sub_cumh_sel2_collow - 1;
			d_in2_sqr_sub2[(bx*d_common->in2_sqr_sub2_elem) +ei_new] = d_in2_sub_cumh[(bx*d_common->in2_sub_cumh_elem) + ori_col * d_common->in2_sub_cumh_rows
				    + ori_row];
		...
		while (ei_new < d_common->in2_sub2_elem)
		{
			...
			// subtract
			d_in2_sqr_sub2[(bx*d_common->in2_sqr_sub2_elem) +ei_new] = d_in2_sub_cumh_sel[(bx*d_common->in2_sub_cumh_sel_elem) +ei_new]
				- d_in2_sqr_sub2[(bx*d_common->in2_sqr_sub2_elem) +ei_new];
		...
		while (ei_new < d_common->in2_sub2_elem)
		{
			...
			temp = d_in2_sub2[(bx*d_common->in2_sub2_elem) +ei_new];
			temp2 = d_in2_sqr_sub2[(bx*d_common->in2_sqr_sub2_elem) +ei_new] - (temp * temp / d_common->in_elem);
			if (temp2 < 0)
			{
				temp2 = 0;
			}
			d_in2_sqr_sub2[(bx*d_common->in2_sqr_sub2_elem) +ei_new] = sqrt(temp2);
		...
		while (ei_new < d_common->in_sqr_elem)
		{
			...
			temp = d_in[ei_new];
			d_in_sqr[(bx*d_common->in_sqr_elem) +ei_new] = temp * temp;
		...
		while (ei_new < d_common->in_sqr_rows)
		{
			...
			for (i = 0; i < d_common->in_sqr_cols; i++)
			{
				...
				sum = sum + d_in_sqr[(bx*d_common->in_sqr_elem) +ei_new + d_common->in_sqr_rows * i];
		...
		while (ei_new < d_common->in2_sub2_elem)
		{
			...
			d_in2_sqr_sub2[(bx*d_common->in2_sqr_sub2_elem) +ei_new] = d_in2_sqr_sub2[(bx*d_common->in2_sqr_sub2_elem) +ei_new] * denomT;
		...
		while (ei_new < d_common->conv_elem)
		{
			d_conv[(bx*d_common->conv_elem) +ei_new] = d_conv[(bx*d_common->conv_elem) +ei_new] - d_in2_sub2[(bx*d_common->in2_sub2_elem) +ei_new] * in_final_sum
				/ d_common->in_elem;
		...
		while (ei_new < d_common->in2_sub2_elem)
		{
			...
			d_in2_sqr_sub2[(bx*d_common->in2_sqr_sub2_elem) +ei_new] = d_conv[(bx*d_common->conv_elem) +ei_new] / d_in2_sqr_sub2[(bx*d_common->in2_sqr_sub2_elem) +ei_new];
		...
		if (d_frame_no == 0)
		{
			tMask_row = cent + d_Row[d_unique[bx].point_no] - d_Row[d_unique[bx].point_no]
				- 1;
			tMask_col = cent + d_Col[d_unique[bx].point_no] - d_Col[d_unique[bx].point_no]
				- 1;
		}
		else
		{
			pointer = d_frame_no - 1 + d_unique[bx].point_no * d_common->no_frames;
			tMask_row = cent + d_tRowLoc[pointer] - d_Row[d_unique[bx].point_no] - 1;
			tMask_col = cent + d_tColLoc[pointer] - d_Col[d_unique[bx].point_no] - 1;
		}
		...
		while (ei_new < d_common->tMask_elem)
		{
			...
			if (ei_new == location)
			{
				d_tMask[(bx*d_common->tMask_elem) +ei_new] = 1;
			}
			else
			{
				d_tMask[(bx*d_common->tMask_elem) +ei_new] = 0;
			}
		...
		while (ei_new < d_common->mask_conv_elem)
		{
			...
			for (ja = ja1; ja <= ja2; ja++)
			{
				...
				{
					ib = ip1 - ia;
					s = s + d_tMask[(bx*d_common->tMask_elem) +d_common->tMask_rows * (ja - 1) + ia - 1] * 1;
				}
			}

			// //d_mask_conv[(bx*d_common->mask_conv_elem) +d_common->mask_conv_rows*(jc-1)+ic-1] = s;
			d_mask_conv[(bx*d_common->mask_conv_elem) +ei_new] = d_in2_sqr_sub2[(bx*d_common->in2_sqr_sub2_elem) +ei_new] * s;
		...
		while (ei_new < d_common->mask_conv_rows)
		{
			...
			for (i = 0; i < d_common->mask_conv_cols; i++)
			{
				...
				largest_value_current = fabs(d_mask_conv[(bx*d_common->mask_conv_elem) + largest_coordinate_current]);
		...
		if (tx == 0)
		{
			...
			pointer = d_frame_no + d_unique[bx].point_no * d_common->no_frames;
			d_tRowLoc[pointer] = d_Row[d_unique[bx].point_no] + offset_row;
			d_tColLoc[pointer] = d_Col[d_unique[bx].point_no] + offset_col;
	...
	if (d_frame_no != 0 && (d_frame_no) % 10 == 0)
	{
		...
		// update coordinate
		loc_pointer = d_unique[bx].point_no * d_common->no_frames + d_frame_no;
		d_Row[d_unique[bx].point_no] = d_tRowLoc[loc_pointer];
		d_Col[d_unique[bx].point_no] = d_tColLoc[loc_pointer];
		...
		while (ei_new < d_common->in_elem)
		{
			...
			// update template
			d_in[ei_new] = d_common->alpha * d_in[ei_new] + (1.00 - d_common->alpha)
				* d_frame[ori_pointer];
